function [ PersistentState, LogObservationLikelihood, xnn, Ssnn, deltasnn, taunn, nunn, wnn, Pnn, deltann, xno, Psno, deltasno, tauno, nuno ] = ...
    ESTNLSSTempKalmanStep( m, xoo, Ssoo, deltasoo, tauoo, nuoo, RootExoVar, diagLambda, nuno, Parameters, Options, PersistentState, StateVariableIndices, t )
...
%     tauno = [];
    ...
    nm = length( Observed );
    if nm > 0
        [ PersistentState, EndoSimulation, MeasurementSimulation ] = StochasticVolatilitySimulation( Parameters, PersistentState, StatePoints, ExoPoints, t );
        
        if any( ~isfinite( EndoSimulation( : ) ) )
            error( 'ESTNLSS:NonFiniteSimultationESTNLSSTempKalmanStep', 'Non-finite values were encountered during simulation in Kalman step.' );
        end
        ...
        [ PersistentState, EndoSimulation ] = StochasticVolatilitySimulation( Parameters, PersistentState, StatePoints, ExoPoints, t );
